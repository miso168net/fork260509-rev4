...
supersedes: []
superseded_by: []
provenance: "rev4:2026-07-31 020-email-verify-smtp brainstorm——四鏡頭研究（wf_471a7c74）＋user 親決 9 題（含衛星表取代加欄、created_{at,by} 成對兩次 user 主動升級）"
...
3. **已驗證值衛星表 `sys_user_email_verify`**（取代 sys_user 加欄——sys_user 從未加欄、功能態
   ...
   失配→自動未驗證，任何現在與未來寫入路徑零清除義務（結構保證、非程序紀律）。
...
7. **發碼節流**：常數寫死（冷卻 60s SET NX EX＋每日 10 次/user INCR EXPIRE）；redis 不可用
   fail-closed；不掛 login throttle 狀態機（計數與狀態機綁死 login 語意、移植即破口——B-102
   結構原因同源）。唯一衝突誠實回報（防枚舉 collapse 口徑僅屬 login 家族）。

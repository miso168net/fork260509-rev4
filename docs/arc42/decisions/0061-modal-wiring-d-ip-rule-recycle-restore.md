...
013-ip-rule-admin 建 IP 規則管理面（MODAL-WIRING (e)「同 manage 範式新管理頁」主軌道）。回收桶面
由兩件控件承載（D1＋clarify Q1）：①**混排單清單**（後端 getIpRuleList 為 hybrid 端點、含軟刪列
active 沉頂 deleted 殿後，前端以 deleted NTag 欄辨識、已刪列只顯復原鈕）②**搜尋卡「狀態」三態過濾**
（現役／已刪除／全部，預設全部）——★**無 menu/user 式的「顯示已刪除」toggle，該視圖切換職能改由
狀態三態下拉承載**（clarify Q1 追加；功能上等價於 toggle＝可切出「已刪除-only」視圖）。③逐列 restore 鈕。
...
- **擴 MODAL-WIRING (d) 用途字串**加「IP 規則回收桶復原」、錨點加 `views/manage/ip-rule/index.vue`
  混排清單（含已刪列顯示與狀態欄辨識）＋其搜尋卡**狀態三態過濾控件**＋逐列 restore 鈕。標頭 (a)~(i)
  用途款集**不新增**（本次只擴 (d) 既有款字串、不立新用途 (j)）。
- **after 逐字終稿**（accepted 時據此改 constitution §III.2 MODAL-WIRING (d) 款，防省略號壓縮丟字面）：
  ...
  之搜尋卡「狀態」三態過濾控件（現役／已刪除／全部——承載已刪視圖切換、取代 toggle 形）＋逐列
  restore 鈕＋對應 i18n key——嚴格限「選單樹復原／父層級調整／使用者回收桶復原／IP 規則回收桶復原」」
  （★ip-rule 以**狀態三態下拉**取代 menu/user 的 toggle 形承載已刪視圖切換，故字面明寫；顯示面與視圖
  切換面一併納入授權字面、不落 (d)/(e) 之間縫隙）。
- **版本**：MINOR（既有款擴字串／擴錨點；§V.3「軌道授權邊界擴展（新用途／新範圍）」＝MINOR，
  v1.8.0/v1.9.0 同判例）；v1.10.0→**v1.11.0**。
- **不需其他 amendment**：搜尋卡之**網段模糊＋類型精確**兩維＝(e) 鏡像 user 頁（★其**狀態三態維**因承載
  已刪視圖切換、已納入上方 (d) 錨點字面、不另計）；hasAuth 按鈕＝(b)；DELETE 接線＝WRAPPER 新檔

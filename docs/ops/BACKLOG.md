...
- B-125｜**host 側暫存／落點目錄命名雙軌未統一**：新資產 `deploy/generate-age-key.sh` 用 **repo 目錄名** `~/.cache/fork260509-rev4`（同機並存 fork260509-rev1~rev4 四代與其他專案、repo 名才零撞名），而既有三處仍用短代號家族——`~/.cache/rev4-secrets`（**SECRETS_DIR 拍板值**、ADR 0080 決策 1、.env 與 compose 掛載來源）／`rev4-decrypt.XXXXXX`（decrypt-secrets.sh mktemp）／`rev4-merge.XXXXXX`（RUNBOOK §15.7 mktemp）。**刻意未統一**：改落點值＝改拍板＋重跑落點遷移（down→decrypt→up→逐容器 inspect→才刪舊落點、契約 §P6 五步），而只改兩個 mktemp 模板反而與拍板值不一致。候選修法＝一次性遷移，把三處與新資產統一到 `~/.cache/fork260509-rev4/` 樹下（secrets／decrypt／merge／keygen 各一層），須附 Risk／Guard／Rollback 三欄表並同刀改 ADR（新 ADR supersede 0080 決策 1 之落點值）｜★**拍板級**——下次動 SECRETS_DIR、或新增第二個 host 暫存用途時一併呈報 user｜出處：019 收刀（user 指出命名應用 repo 名、2026-07-30）

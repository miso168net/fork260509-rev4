...
- B-104｜★上游 bug：快速連點左側 menu 後 main（#__SCROLL_EL_ID__）永久空渲染——Vue BaseTransition（global-content 之 Transition mode=out-in＋KeepAlive）快速連續導航 race 使 state.isLeaving 卡 true、render 永久回空佔位；指紋＝main 僅剩 teleport/v-if 註解節點＋appStore.contentXScrollable 恆 true；後續點 tab/menu/內建刷新鈕（reloadFlag toggle）全無效、★唯一復原＝整頁 F5。重現法＝~80ms 間隔連續 router.push 14 次（40ms 不中——須正中 fade-slide ~300ms leave 窗；2026-07-17 CDP 新分頁實證）。歸屬＝上游（global-content/index.vue 上游原檔零 fork 改動、Vue 3.5.34 lockfile 版本、rev3 同構同病；vuejs/core out-in＋KeepAlive 已知 bug 族）。修法候選＝改 Transition 去 out-in 或導航防抖——★layout 檔不在憲法 §III.2 (a)~(i) 授權用途內、動它需 amendment/ADR 治理級親決｜upstream rebase 時覆核是否已修、或使用者實際受擾再排刀（user 拍板 A 案 2026-07-17）｜出處：user 實機回報＋CDP 診斷

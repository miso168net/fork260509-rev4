...
- B-087｜sys_menu 測試家族偶發假紅（010 committed-row 併發測與計數型斷言預設平行互擾；已見二例：①list_tree_governed_source_and_top_level_pagination 單 txn 兩次 list_tree total 15 vs 14〔011 U2 審查〕②list_active_returns_seeded_menus 斷言 seed 78 啟用選單轉紅一次〔011 U3 審查〕；修法候選＝併發測加 #[ignore] 序列跑、或計數斷言改容忍並行位移/同語句快照）｜該假紅再現時（L-138 近緣）

...
- B-059｜settings 頁 tooltip 三語化：IconTooltip 直顯 DB seed 繁體 description（index.vue 行 170、zh-CN/en 語系下不符）→ 15 鍵 help 三語鍵化（page.manage.systemSettings.help.*）＋tooltip 改 $t、seed description 留 fallback、零 migration 零後端（★2026-07-16 013 收刀後重偵察：條目原述 8 鍵時代「7/8 同文＋005 遺漏 idle label」已漂移——15 鍵 labelKeyMap 全映射、idle label 已由 008 補〔碼註記 B-059 部分消化〕；殘餘僅 tooltip 一處、量級縮至單一單元）｜下一刀（2026-07-16 user 定向）｜出處：004 單元⑧ user 拍板 B

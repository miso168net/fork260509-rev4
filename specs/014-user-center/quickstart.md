...
- **updateProfile**：全 None no-op（零時戳 bump）、值域外 gender 不動、部分更新零串擾、鎖內查無 notFound；get_own_profile 標的軟刪→Internal 5000。
- **固定序拒因**：userNotFound→passwordMismatch→oldPasswordMismatch→passwordSameAsOld→policy violations 各自可達且序正確（正典序）；鎖內 phc 已變→oldPasswordMismatch（並發改密）。
- **getPasswordPolicy allowlist 負向**：非 password_ 前綴鍵（session_／ip_ 類）不出現於回應——FR-010 負向面／SC-007 後半機器背書。
- **redact**：ChangePwdReq Debug 輸出零明文；op-log payload 零密碼。
- 結構性承載註記：廣播失敗存活窗（島 I2 既有明文接受）與政策快照即時調整（load_policy 單快照既有機制）不另立測。
...
| S3 | 拒因觸發（舊密錯／confirm 不一致／新同舊／政策違規多條；★第五類帳號消失＝並發刪帳、CDP 不觸發、由 T008 固定序單元測代驗） | CDP 四類各自明確可區分；政策違規 toast 逐條明細（SC-003 五類對帳＝CDP 四＋單元測一）；表單即時提示：政策 6+1 鍵＋confirm＋新同舊 |

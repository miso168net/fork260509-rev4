...
| ①c | high-5xx | 5xx 比率（axum_http_requests_total）大於門檻、for 5m | noDataState=OK＋`or vector(0)`＋clamp_min（防雷⑨） |
...
| ⑤ | reaper 心跳超時 | `time()-reaper_last_success_timestamp{mode="execute"}` 大於 2×間隔 | 只認 execute、同源門檻 |

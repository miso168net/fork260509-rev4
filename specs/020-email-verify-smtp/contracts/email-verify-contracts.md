...
- 勘註（U9、user 2026-08-01 拍板佈局改版；U10 措辭誠實化）：captcha 圖與答案輸入整組移入
  Send Code Layer 浮窗（NModal、沿 pwd-gen-modal 先例形）；卡面右欄改 Phone 同構三件式
  （Send Code 開浮窗＋碼輸入常駐＋Verify）；浮窗標題組合式（sendCode＋字面「 - 」＋emailTitle、
  零新鍵共用設計）——行為保留清單（點圖換題／倒數 remainingSeconds 重建／回填態清理等）＋兩點
  有意變化：①取題時機改開層時（頁載零取題）②冷卻中卡面無 captcha 圖（圖僅存在於浮窗內；卡面倒數承載
  狀態）。

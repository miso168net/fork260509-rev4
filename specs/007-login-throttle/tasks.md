...
- **[P]**：可平行（不同檔、無未完成依賴）。★**rust build/test 一律容器內、全程 serial**——`[P]` 指「可分派給不同執行單元」，**不是**平行跑 cargo。
...
- [ ] T009 [P] 於 `rust-api/server/src/model/facade/sys_login_attempt.rs` 新增 `count_recent_failures<C: ConnectionTrait>(conn, user_name, window_minutes, unlock_marker_ts: Option<..>) -> Result<i64, DbErr>`：raw SQL 單 statement（`Statement::from_sql_and_values` + `query_one` + `try_get::<i64>("", "c")`），實作 data-model §5.3 的 `GREATEST` 三源下界＋帶窗下界的相關 scalar subquery；★函式內頂端加 **測試先決①** 的 `#[cfg(test)]` `thread_local` DbErr 注入旗標＋RAII guard（**非** `AtomicBool`）；★註解寫明「無 marker → 綁 SQL NULL 安全（Postgres `GREATEST` 非 strict、忽略 NULL），**不得用 sentinel**」，並附守門測試
- [ ] T010 [P] 於 `rust-api/server/src/` 測試支援模組新增 **測試先決②** 結構化告警捕捉層：自訂 `tracing_subscriber::Layer::on_event` + `field::Visit` 收集 `metadata().target()` 與 `degraded` 欄；`registry().with(layer)` + `set_default` 取 `DefaultGuard`（★零新依賴；降級守門一律用 `#[tokio::test]` 預設 `current_thread`）
- [ ] T011 [P] 於 `rust-api/server/src/handler/auth.rs` 的 `#[cfg(test)] mod tests` 新增 **測試先決③** raw SQL 種 `created_at` 的 helper（照 `seed_rotated_token` 的 raw INSERT…RETURNING 形；★`real_ip` 為 `inet NOT NULL`、必給）
...
- [ ] T013 [P] 於 `rust-api/server/src/handler/auth.rs` 的 `mod tests` 複製 `bad_redis()`（★原住 `auth/enforce.rs` 的 test mod、跨模組不可見）
- [ ] T014 [P] 於 `rust-api/server/src/redis/mod.rs` 新增集中 key-builder `throttle_dim_key(dim, value)`／`throttle_captcha_used_key(nonce)`（L-081 單一 helper 導出），以及 `SET NX EX` 單次標記 helper 與麵包屑（`SET NX EX 60` → `INCR` → `GETDEL`）helper；沿用 R7 嚴格分流底座
...
- [ ] T018 建立 `rust-api/server/src/throttle/` 模組骨架：活書常數（`THROTTLE_LOCK_TTL_SECS=900`／`CAPTCHA_TTL_SECS=300`／`CAPTCHA_ANSWER_LEN=4`／`DEFAULT_MAX_FAILS=5`／`DEFAULT_WINDOW_MINUTES=15`／`DEFAULT_CAPTCHA_AFTER=2`／`LOGIN_USER_NAME_MAX=64`／`LOGIN_PASSWORD_MAX_BYTES=512`）、三門檻鍵解析＋fail-default、結構化告警 helper `warn!(target: "security.throttle", degraded = ..)`＋`metrics::counter!("throttle_degraded_total", "source" => ..)`（★固定小集合 label、勿加高基數）（依賴 T014/T015）
...
- [ ] T021 [P] [US1] 於 `rust-api/server/src/handler/auth.rs` 的 `mod tests` 加 reset-on-success 守門（savepoint 單連線：錯 4 → 成功 → 錯 4 → **不鎖**）
- [ ] T022 [P] [US1] 於同檔加**窗過期自癒**守門（以 T011 helper 種 6 筆 `created_at = now() - (window + ε)` 的失敗列 → 登入**不被鎖**）
- [ ] T023 [P] [US1] 於同檔加 **L1 只由③武裝**守門（錯 `max_fails` 次後 `throttle:lock:*` key **不存在**；下一請求走③後才存在）
- [ ] T024 [P] [US1] 於同檔加**假鎖不發生**守門（B-066 雙 committed 連線：錯 4 次 → 並發〔成功×1 ＋ 失敗×1〕→ 斷言無 L1 假鎖、成功者可再登入）
- [ ] T025 [P] [US1] 於同檔加**計數 race 有界超越**守門（雙連線並發錯密 → **delta 上界 `≤N` 斷言**；★L-079 禁絕對計數）
- [ ] T026 [P] [US1] 於同檔加**防枚舉不變式**守門（不存在帳號 vs 真帳號，在 collapse／lock 兩路徑碼與訊息完全相同）
- [ ] T027 [P] [US1] 於同檔加 **FR-022 形制上限**守門（`user_name` 65 字元／`password` 513 bytes → `1000`、**零稽核列、零 argon2、不消耗計數桶**）
...
- [ ] T032 [P] [US2] 於 `rust-api/server/src/handler/auth.rs` 的 `mod tests` 加 **captcha 零計數**守門（灌 N 個「無 captcha 的軟區請求」→ 窗內失敗列數**恆不變**、鎖定**不觸發**）
- [ ] T033 [P] [US2] 於同檔加**提交即消耗**守門（同一 `captchaId` 先帶錯誤答案提交 → 再帶正確答案提交 → **第二次亦拒**）
- [ ] T034 [P] [US2] 於同檔加**重放**守門（同一 `captchaId` 帶正確答案二次提交 → 第二次拒）
- [ ] T035 [P] [US2] 於同檔加**跨帳號**守門（為 A 簽的 challenge 用於 B → 拒，且 `used` 標記**未被寫入**）
- [ ] T036 [P] [US2] 於同檔加**簽章／exp 失敗**守門（竄改 `captchaId`／`exp` 過期 → 拒，且**不消耗**）
- [ ] T037 [P] [US2] 於同檔加**答案不可還原**守門（僅憑 `captchaId`、無 secret，對 36⁴ 空間暴力 → 全數失敗）
- [ ] T038 [P] [US2] 於同檔加**硬鎖優先**守門（鎖中附**有效** captcha → 仍 `2222 locked`，且該 `used` 標記**未被寫入**）
- [ ] T039 [P] [US2] 於同檔加**未達軟區忽略**守門（`count < captcha_after` 時附 captcha → 不驗、不消耗；該題稍後於軟區**仍可用**）

### 實作（後端）

- [ ] T040 [US2] 建立 `rust-api/server/src/captcha/` 模組：以 `captcha` crate 產圖（`set_chars` 明列 36 字英數、`add_chars(4)`、`as_base64()` → `data:image/png;base64,`；★實作期驗證內嵌字型涵蓋所選字集）；nonce 以 `argon2::password_hash::rand_core::OsRng` 產生（★**不引入 `uuid`／`rand` crate**，沿 `uuid_v4()` 既定 pattern）
...
- [ ] T042 [US2] 於 `rust-api/server/src/handler/throttle.rs` 新增 `login_captcha` handler（`GET /auth/loginCaptcha?userName=`；★對任意 `userName` 一律發題、零存在性洩漏；★`userName` 亦受 FR-022 形制上限約束；★**產題對 Redis 零寫入**）
...
- [ ] T049 [US2] 於 `base-web/src/views/_builtin/login/modules/pwd-login.vue` 實作驗證碼條件渲染／點圖換題／帳號名變更重取／**答錯自動重取**；★本檔為**首個 fork-delta**——`<script>` 區用 `//` 標記、**`<template>` 區用 `<!-- [rev4-inline ★LOGIN-CAPTCHA-WIRING(i) 007-login-throttle] 原行: … -->`**；改動後**立即跑 `tools/fork-delta-lint`** 驗證 template 標記被接受（依賴 T047、T048）
...
- [ ] T053 [P] [US3] 於 `rust-api/server/src/handler/auth.rs` 的 `mod tests` 加**解鎖生效**守門（鎖定 → 解鎖 → 下一擊不被舊列 re-lock）
- [ ] T054 [P] [US3] 於同檔加**冪等／非永久豁免**守門（解鎖後再累積至門檻 → 正常重新鎖定）
- [ ] T055 [P] [US3] 於同檔加 **op-log 恰一列**守門（★**delta 斷言**——Super 為共享實體，L-079 的原始場景）
- [ ] T056 [P] [US3] 於同檔加**動作序負向自證**守門（把序反過來〔先 `DEL` 後 `SET`〕→ race 重現、守門轉紅）
...
- [ ] T059 [US3] 於 `rust-api/server/tests/contract.rs` 的 `registry()` 加 `unlock-login` case（沿 `assert_protected_no_token_3333` 範式）
- [ ] T060 [US3] 於 `rust-api/server/src/model/facade/sys_login_attempt.rs` 的 `count_recent_failures` 消費解鎖標記（`unlock_marker_ts` 進 `GREATEST` 下界；★無 marker 綁 SQL NULL）並補守門測試（依賴 T009）
...
- [ ] T061 [P] [US4] 於 `rust-api/server/src/handler/auth.rs` 的 `mod tests` 加降級源①②⑤⑦守門（以 T013 的 `bad_redis()` 注入；斷言退 L2／captcha 要求停用／拒絕但零計數／★re-lock 唯一例外／忽略；★各斷言對應 `degraded=` 結構化 warn 出現，以 T010 捕捉層驗）
- [ ] T062 [P] [US4] 於同檔加降級源③守門（以 T009 的 `cfg(test)` 旗標注入 `DbErr` → `count=0` 放行 ＋★`captcha_forced`〔redis 可用時無條件要求 captcha〕＋`degraded=db_count`）
- [ ] T063 [P] [US4] 於同檔加降級源④守門（`record_attempt` 注入 `DbErr` → 登入回應不變 ＋`degraded=db_write`）
- [ ] T064 [P] [US4] 於同檔加降級源⑥守門（savepoint 內 `DELETE` 設定列 → 退預設常數 5/15/2 ＋`degraded=settings_default`）
- [ ] T065 [P] [US4] 於同檔加 `state.redis = None`（test-stub）守門（視同 `redis_down`：純 L2 ＋ captcha 要求停用）
...
- [ ] T069 [P] [US5] 於 `rust-api/server/src/handler/auth.rs` 的 `mod tests` 加**稽核邊界六情境**守門（★全部以 **delta 斷言**、禁絕對計數，L-079）
...
- [ ] T078 CDP-2（驗證碼 UI）：`login_throttle_captcha_after` 暫調為 `1` → 錯密一次 → 驗登入頁出現驗證碼圖與輸入欄、**可點圖換題**（`captchaId` 改變）→ 收尾同上；★CDP **不驗證「答對」路徑**（答案不可還原、瀏覽器端無從取得）
...
- [ ] T081 [P] 以 `python3 tools/docs-sync errata B-019` 機器枚舉全 repo 同語意命中，逐處把 B-019 條目措辭勘誤為「rev3 有完整三層信任模型（peer-gate → CDN 位置錨 → rightmost-untrusted），`internal_default` 僅 Tier-2 skip 集之一參數、住 operator TOML；rev3 REVIEW 將『寬 internal_default 信任』登記為 accepted 取捨、非 open 缺陷」
...
- [ ] T084 容器內跑 `cargo test --workspace` 全綠（serial）；`schema-gate` gate1／gate2／audit 三閘綠；`entity_access_lint`／`fork-delta-lint`／locale 對等／`wire_schema`／契約覆蓋閘 全綠；`python3 tools/docs-sync check` 一致
...
**任務內硬依賴**：T003←T002｜T007←T006｜T012←T003｜T018←T014,T015｜T020←T007,T019｜T028,T029←T018｜T041←T003,T040｜T045←T029,T041｜T049←T047,T048｜T051←T047｜T057←T014,T016,T017｜T060←T009｜T066←T029,T045｜T067←T018｜T075←T074｜T083←T046,T059

## Parallel Opportunities

- **Phase 1**：T005／T006 可平行（不同檔）。
- **Phase 2**：T009／T010／T011／T013／T014／T015／T016／T017 八支可平行（★皆不同檔）。
- **Phase 3**：T021~T027 七支測試可平行撰寫；T030（前端 i18n）與後端實作平行。
- **Phase 4**：T032~T039 八支測試可平行；T047／T048 前端新檔可平行。
- **Phase 6**：T061~T065 五支降級測試可平行。
- **Phase 9**：T079／T080／T081／T082 四支文件任務可平行。

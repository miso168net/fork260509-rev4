...
### Session 2026-07-16（/speckit-clarify）

- Q: 搜尋卡要不要加「狀態（現役／已刪除）」過濾維度？ → A: 加——搜尋卡增第三維「狀態」三態下拉（現役／已刪除／全部），`getIpRuleList` 契約相應加 `deleted` filter 參數（tri-state、預設「全部」＝維持 D1 混排全景不改預設行為）。理由：直接篩已刪規則、改善混排尾頁可達性（原 §10 已知取捨的解法）。

## User Scenarios & Testing *(mandatory)*

### User Story 1 - 超管檢視並搜尋 IP 規則清單（含已刪、見審計資訊） (Priority: P1)

超級管理員進入「IP 規則」管理頁，看到一份混排清單：現役規則沉頂、已刪規則殿後，每列顯示網段、類型（放行／阻擋）、備註、排序值、狀態（現役／已刪除）、建立/更新時間與操作者。可用搜尋卡以網段模糊比對＋類型精確過濾＋狀態三態（現役／已刪除／全部）快速定位。
...
- **混排＋分頁可達性**：active 沉頂使已刪列於「全部」視圖恆居尾頁——已刪規則多時，超管改用狀態下拉切「已刪除」直接篩出（clarify Q1＝加狀態維度之動機）；「全部」視圖仍維持混排全景為預設。
...
- **FR-002**：系統 MUST 提供搜尋卡三維：網段（`wbipCidr`）模糊比對（大小寫不敏感、子字串）＋類型（`wbipType`）精確過濾（allow／deny、可清空）＋狀態（`deleted`）三態選擇（現役／已刪除／全部）；空搜尋條件等同不過濾（狀態預設「全部」＝現役與已刪混排）。
- **FR-003**：`getIpRuleList` 契約 MUST 擴充加入上述三個可空 filter 參數（`wbipCidr` 模糊、`wbipType` 精確、`deleted` tri-state）；`deleted` 三態語意＝現役-only（`deleted_at IS NULL`）／已刪-only（`deleted_at IS NOT NULL`）／全部（不加此條件、維持 hybrid 排序）；查詢 registry 簽名 MUST 同步更新並以契約測試斷言。
...
- **SC-002**：以網段片段搜尋（含單主機規則的顯示值與「/32」）皆能命中對應規則；類型過濾精確；狀態三態（現役／已刪除／全部）各自只列對應集合、分頁 total 隨之誠實。
...
- 混排單清單＋後端分頁為預設視圖（狀態＝「全部」）；已刪列於該視圖恆居尾頁，另以狀態三態下拉直接篩選（clarify Q1；異於 rev3 兩維搜尋）。

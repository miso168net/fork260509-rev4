...
| P4.6 | 落點自建 **0700 子目錄** | ★原理由「`/dev/shm` 為 `drwxrwxrwt`（world-writable）」屬 2′ 原值、隨重拍作廢（2026-07-29、#11 反轉後改 `$HOME/.cache/rev4-secrets`＝`drwx------`；ADR 0080）；要求維持＝縱深防禦、與落點無關——否定測試：落點父目錄權限非 0700 亦然，解密後 `stat` 子目錄必為 `700`（S6 併驗） |

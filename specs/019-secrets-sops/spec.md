...
5. **Given** `wsl --shutdown` 重開機且未跑解密儀式，**When** 跑 preflight，**Then** 明確紅
   （指名缺檔）而非服務靜默啟動失敗。
   ★重拍（2026-07-29、#11 反轉後）：SECRETS_DIR 改解法 2（ext4 持久）後**重開機不再清空落點**，
   原觸發條件（重開機造成缺檔）結構性不成立——照原文驗收只會得到假綠。**觸發條件改為「落點
   缺檔」**：**Given** 明文尚未解密或落點被清空（`rm -rf "$SECRETS_DIR"`），**When** 跑 preflight，
   **Then** 明確紅（指名缺檔）而非服務靜默啟動失敗。機判劇本＝tasks T039 與 quickstart S4 後半
   之否定測試（兩處已為此形、無需連帶改）。
   反向性質「`wsl --shutdown` 後明文仍在、compose 直接可起（免開機儀式）」＝解法 2 的設計後果、
   登記於 ADR 0080「後果」節，**不升格為 US3 機判驗收項**——驗證需真的 `wsl --shutdown`
   （破壞性、屬人工一次性確認），升格只會製造無對應 task 的驗收項。
...
- B′ 日常摩擦（每次解密輸 passphrase）與 2′ 開機儀式（重開機後重跑解密）＝已拍代價。
  ★重拍（2026-07-29、#11 反轉後）：**2′ 開機儀式代價隨 2′ 作廢**（解法 2 ext4 持久＝免開機
  儀式）；B′ 日常摩擦維持不變；新增之已拍代價＝**明文長駐 `ext4.vhdx`（at-rest 面）**，補償面
  ＝①私鑰 B′ 使根信物不隨檔案讀走②三層掃描防線擋入庫面③RUNBOOK §4 BitLocker 確認項
  （ADR 0080「後果」節）。

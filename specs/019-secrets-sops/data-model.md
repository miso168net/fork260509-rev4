...
`SECRETS_DIR=` 一行**（非法值吵鬧失敗、不靜默回退）→ 皆缺回退 `deploy/secrets`；③「環境變數
優先」之**空字串邊界**：**已匯出但為空** ≠ 未設（compose 對空字串直接吃 `${…:-…}` 預設值、
不讀 `.env`），五支賦值型解析器一律**吵鬧失敗**、不得續讀 `.env`（019 U4 quality 第 3 輪；詳 P5.1）；
④「環境變數優先」之**相對值錨定基準＝repo 根**（非 CWD；compose 以專案目錄解析——五支不同錨
即同一個值指向兩個目錄且雙方各自 `rc=0`，019 U4 quality 第 4 輪、**L-178**；詳 P5.1）。

...
③續修），故終點改取任何較新 HEAD 重跑必得 `drift=['L-197']` 一筆——此為**就地勘誤的預期結果、
非分卷缺陷**（分卷只搬動不改內容；聯集與交集兩項在 HEAD 仍全綠＝遺失 0／新增恰 L-199）。要驗分卷
本身：終點取 `a4b6bff`，或把 `L-197` 列入排除清單。主卷回落至 **14007 tokens**（量測時點

...
節標題 4→8。**分卷歷程（★下列 token 數皆是量測時點值、會被後續輪次自己的 append 推翻）**：整理＋
...
切前集合、交集為空、逐筆 byte 級零漂移——切時全綠，並經 U6 收單確認**獨立重跑**全綠（實測
24＋25＝49＝切前 48 筆＋同 commit 新 append 之 L-199 一筆；聯集比對須把 L-199 併入期望集合，
否則會誤判成「多一筆」）；主卷回落至 **14007 tokens**（量測時點 07f1df7＝42021 bytes÷3）、
`lint` 回到 **0 警告**。

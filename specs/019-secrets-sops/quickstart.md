...
`python3 tools/docs-sync.py lint | tail -3` 為準、不以本行任何數字為基準——★U6 兩次實證＝
**L-197 失效模式雙向復發**：先是 T038 把 `3415dc9` 當時的 21598／0 警告寫成終驗恆值、被同單元
後續兩輪自己的 append 推翻；接著本段又把「恰只允許 1 筆警告」寫成收刀恆值、被第 4 輪自己的
分卷推翻（**警告被消滅也是一種推翻**——故筆數不入判準、詳 L-197 防法②）。

...
| 驗收 | 正向 | 否定測試（必須紅） |
|---|---|---|
| key 斷言 (c) | 完整 enc 檔 → 全部寫出 | **刪 enc 檔一個 key** → 零寫入＋非零退出＋指名缺哪個 key（**絕不落到造亂數路徑**） |
| `.new` 守衛 (e) | 現值＝解密值 → 直接寫入 | 構造 `alert_webhook_url` 差異 → 產生 `.txt.new`＋警示、**原檔不變** |
| 無尾端換行 (a) | `xxd \| tail -1` 尾端**無 `0a` 無 `0d`** | 注入 CR 的檔 → preflight CR 護欄轉紅、修正後復歸 |
| byte-identical (a) | leaf byte 數與 composite 內嵌值一致 | 手改 leaf 不重跑 → 一致性檢查紅 |
| 權限 (b) | 目錄 700／檔 644／owner＝本人 uid:gid | 觀察是否出現 `root:root`（P1.6 對策失效的信號） |
| tty 守衛 (b) | 互動情境正常 | 非互動呼叫 → **吵鬧失敗**（不得 hang、不得寫出帶 CR 的檔） |
...
**做什麼**：量測 pre-commit 端到端延遲（外框＋掃描器＋值比對），比較基準＝018 現況。

**方法**：沿 L-155 方法論——drvfs 牆鐘變異大，取多次中位數、與同機同日基準對照；記錄
「純碼 commit（幾乎全 skip）」與「治理檔 commit（全條款跑）」兩情境。

**期望**：維持秒級；若超標，記錄成本結構並掛 BACKLOG（比照 018 的 SC-008 處置慣例）。

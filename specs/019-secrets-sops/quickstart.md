...
★**「全綠」不等於「0 警告」**（U6 quality 第 3 輪釐清）：收刀當下**允許且恰只允許 1 筆警告**
＝`L7｜docs/ops/LESSONS.md｜約 24783 tokens 逼近單卷上限 25000`（WARN 門檻 22500 已越、ERROR 未到），
可行動歸屬＝BACKLOG **B-121**；出現**第 2 筆警告或任何錯誤即不放行**。要求 0 警告等於逼本刀分卷，
而 B-121 明載不宜同刀做（L 號仍在增修、切了製造 diff 噪音）。★該 token 數是**量測時點值**：
每輪 fix 若又 append 教訓就會變動，判讀時以現場 `python3 tools/docs-sync.py lint | tail -3` 為準、
不以本行數字為基準（U6 實證：T038 曾把 `3415dc9` 當時的 21598／0 警告寫成終驗恆值，被同單元
後續兩輪自己的 append 推翻）。

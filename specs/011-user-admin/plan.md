...
**Scale/Scope**: 12~14 執行單元（近 009/010 先例）；10 端點＋1 seed migration（m008）＋1 net-new modal（解鎖）＋動既有 auth 流程（run_login/run_refresh）；前端一 ADAPT `.d.ts`＋一 WRAPPER（9 fetcher；getUserList 複用凍結 fetchGetUserList）
...
| Q2 | 動 base-web inline？屬 §III.2 哪個用途？授權邊界內？依 fork-delta 紀律？ | **是、多數落既有軌道，四處超界須隨刀 Amendment**：MODAL-WIRING **(a)** index.vue delete/batchDelete handler＋user-operate-drawer submit 接真 API＋附屬小修（清 mock 補償碼）；**(b)** hasAuth('user:add/edit/delete/kick/reset-pwd/unlock') gating；I18N-WIRING **(i)/(ii)/(iii)** onError 明細渲染＋`backend.biz.user.*`／`backend.biz.unlock.*` 三語＋Schema 型。★**超既有授權者（四處）**：①使用者回收桶「顯示已刪除」toggle＋逐列 restore 鈕——(d) 現文「嚴格限選單樹復原／父層級調整」、射程不含 user 頁；②頁首解鎖 modal（`user-unlock-modal.vue` net-new）＋operate 欄新增 kick/reset-pwd 動作鈕——非 (a) 既有 placeholder、非 (c)「角色×權限維度」、非 (d) 選單域；③編輯抽屜新增表單控件（add 模式密碼欄＋004 政策 hint fetch、edit 模式 session_policy select）——超 (a)「placeholder 接線＋附屬小修」字面（upstream drawer 無此二控件）；④★登入頁 `pwd-login.vue` 密碼/帳號名表單規則放寬 required-only——★**此檔在 `views/_builtin/login/` 而非 `views/manage/**`**，落在所有 MODAL-WIRING（限 manage）／AUTH-WIRING（明文不改 pwd-login）／LOGIN-CAPTCHA-WIRING（限 captcha 渲染）射程外。→ **隨島 I MINOR Amendment 擴 §III.2**：(d) 「顯示已刪除 toggle＋restore」軌道擴至 user 頁；為「頁首維運 modal（解鎖）＋operate 維運動作（kick/reset-pwd）」立新用途；drawer 密碼欄/session_policy select 明確歸類（create/update 接線必要控件或納擴展）；pwd-login 放寬立新登入用途（或擴既有登入軌道）。皆走 fork-delta `rev4-inline`（修改型帶原行／新增型圈界／`.vue` template 用 `<!-- -->`）。 |
...
| Q7 | 觸及 §III ★ 軌道？授權邊界內？補完 or 新能力？ | **是**（同 Q2）。(a)(b)＋I18N-WIRING 屬既有授權範疇（補完）；★**四處跨既有用途邊界的新能力→須 Amendment**（隨島 I MINOR、於前端執行單元前 user 親決）：①回收桶 toggle（擴 (d)）②頁首解鎖 modal＋kick/reset 維運動作（新用途）③drawer 新表單控件（密碼欄/session_policy select、明確分類）④★pwd-login 表單規則放寬（`_builtin/login/`、所有既有登入軌道射程外、立新用途）。解鎖 UI 消費既有 007 unlockLogin 端點（super-only 政策已備）＝純接線。 |
...
**Gate 結論**：通過。需 Amendment 者＝Q9 新島 I 進場（MINOR）＋Q2/Q7 §III.2 軌道擴展（**四處**：(d) 擴 user 頁回收桶 toggle／頁首維運 modal＋operate 動作新用途／drawer 新控件分類／★pwd-login 放寬立新登入用途；順載島 I MINOR Amendment、於前端執行單元前 user 親決）；皆既定治理動作、非違規。三份 ADR draft＋收刀活書 §6 as-built 更新見 Complexity Tracking。

**Phase 1 後複查（設計產物 data-model／contracts／quickstart 產出後）**：九題判定全維持——設計產物一致佐證而非推翻。Q8 確認零 schema 變更（data-model §2：sys_user 17 欄／sys_user_role 2 欄凍結、m008 純 seed 8 列 additive）；Q9 確認島 I 五條與 B-084（島 G5 兌現）、B1（島 B 範式擴充）落地一致；Q2/Q7 確認**四處**超既有 §III.2 用途（回收桶 toggle／解鎖 modal＋kick·reset／drawer 新控件／★pwd-login 放寬）、隨島 I MINOR Amendment（★analyze 補全：初判漏列 drawer 新控件與 pwd-login 放寬，後者在 `_builtin/login/`、所有既有登入軌道射程外）。零新違規、零新 NEEDS CLARIFICATION。Gate 維持通過。
...
├── src/service/api/rev4-user-admin.ts    # ★新增 WRAPPER：9 fetcher（getUserList 複用凍結 fetchGetUserList；不經 barrel、避 vite stale-export）
...
└── src/locales/langs/*                   # 三語：backend.biz.user.* 15 鍵＋backend.biz.unlock.* 2 鍵（007 欠帳）＋page.manage.user 補充鍵＋回收桶 UI 鍵（複用 010/009）＋App.I18n.Schema 型
```
...
| §III.2 軌道擴展（Q2/Q7、**四處**） | ①回收桶 toggle 超 (d) 選單域射程 ②解鎖 modal＋kick/reset 維運動作無既有用途 ③drawer 新控件（密碼欄/session_policy select）超 (a) 字面 ④★pwd-login 放寬在 `_builtin/login/`、所有既有登入軌道射程外（analyze 補全） | 隨島 I MINOR Amendment：(d) toggle/restore 擴至 user 頁；新增維運用途（頁首維運 modal＋operate 維運動作）；drawer 控件明確分類（create/update 接線必要控件或納擴展）；pwd-login 放寬立新登入用途（或擴既有登入軌道）；★於前端執行單元前 user 親決（同 009/010 於 U12 前落 Amendment 先例） |
